...
/**
 * Provides a handle for {@code CardListPanel} containing the list of {@code PersonCard}.
 */
public class CardListPanelHandle extends NodeHandle<ListView<Card>> {
    ...
    private Optional<Card> lastRememberedSelectedPersonCard;

    public CardListPanelHandle(ListView<Card> personListPanelNode) {
     ...
     * @throws IllegalStateException if the selected card is currently not in the scene graph.
     */
    public CardCardHandle getPersonCardHandle(int index) {
        return getAllCardNodes().stream()
                .map(CardCardHandle::new)
    ...
    private Card getPerson(int index) {
