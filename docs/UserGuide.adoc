...
* Words in `UPPER_CASE` are the parameters to be supplied by the user e.g. in
 `add f/FRONT b/BACK`, `FRONT` and `BACK` are parameters which can be used as
 `add f/Hola b/Hello`.
* Items in square brackets are optional e.g `f/FRONT b/BACK [t/TAG]` can be used
...
* Parameters can be in any order e.g. if the command specifies `f/FRONT
b/BACK`, `b/BACK f/FRONT` is also acceptable.
====
